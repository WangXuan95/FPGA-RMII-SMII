...
它的设计代码是 RTL 目录中的 rmii_phy_if.sv 。它是根据规范文档 RMII.pdf 编写的。使用方法详见代码注释。它已在 LAN8720（一个 RMII 接口的 PHY 芯片）上成功运行了网络通信。
...
它的设计代码是 RTL 目录中的 smii_phy_if.sv 。它是根据规范文档 SMII.pdf 编写的。使用方法详见代码注释。它已在 KSZ8041TLI-S（一个 SMII 接口的 PHY 芯片）上成功运行了网络通信。
...
Its design source is [rmii_phy_if.sv](./RTL) in the [RTL](./RTL) directory. It is written according to the specification document [RMII.pdf](./RMII.pdf) . Please refer to the code comments for its usage. It has successfully run ethernet communication on LAN8720 (a PHY chip with RMII interface).
...
- [tb_rmii_phy_if.sv](./SIM) is a testbench for rmii_phy_if.sv, it generates fake, short frame on RMII RX channel, rmii_phy_if will convert it to MII RX waveform. Simutinously, it generates fake, short frames on the MII TX channel, which are converted into RMII TX waveforms by rmii_phy_if.
...
Its design source is [smii_phy_if.sv](./RTL) in the [RTL](./RTL) directory. It is written according to the specification document [SMII.pdf](./SMII.pdf) . Please refer to the code comments for its usage. It has successfully run ethernet communication on KSZ8041TLI-S (a PHY chip with SMII interface).

## RTL Simulation

Simulation related files are in the [SIM](./SIM) folder, where:

- [tb_smii_phy_if.sv](./SIM) is a testbench for smii_phy_if.sv, it generates fake, short frame on SMII RX channel, smii_phy_if will convert it to MII RX waveform. Simutinously, it generates fake, short frames on the MII TX channel, which are converted into SMII TX waveforms by smii_phy_if.
